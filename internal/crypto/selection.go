...
func DecryptText(selection, passphrase string) (string, error) {
	...
	// Frappe masquee : le texte est parcouru d'un bout a l'autre, retours a la
	// ligne compris. Les en-tetes sont reconnus ou qu'ils se trouvent.
	if plain, err := decryptStream(stripLegacyPrefixes(selection), passphrase); err == nil {
		return plain, nil
	}
	if recognized {
		return "", ErrWrongKey
